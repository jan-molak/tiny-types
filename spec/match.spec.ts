...
describe(`::match`, () => {
    ...
    describe('default rules', () => {
        it(`uses the default rule when no identical match is found for a string value`, () => {
            ...
            expect(result).toEqual('got four');
        });
    ...
    describe('when selecting a matcher', () => {
        ...
        it.each([
            ...
            {
                input: new EmaiAddress('user@domain.org'),
                pattern: new EmaiAddress('user@domain.org'),
                expected_matcher: ObjectMatcher
            },
        ...
        ])(`uses a matcher appropriate to the input`, ({ input, pattern, expected_matcher }) => {
            expect(match(input).when(pattern as any, _ => _)).toBeInstanceOf(expected_matcher);
        });
    });
    ...
    describe('when working with TinyTypes', () => {
        ...
        it.each([
            ...
            {
                command: new SuspendAccount(new AccountId(42)),
                expected: 'Command: SuspendAccount(value=AccountId(value=42))'
            },
            { command: null, expected: 'Unrecognised input: null' },
        ])('matches a TinyType by type', ({ command, expected }) => {
        ...
        it.each([
            ...
            { command: null, expected: 'Unrecognised input: null' },
        ])('matches a TinyType by value', ({ command, expected }) => {
            const result = match(command)
